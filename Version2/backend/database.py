        ...
        try:
            with self.engine.connect() as connection:
                ...
                if result.returns_rows:
                    # If it returns rows, fetch them. Handles SELECTs that return 0 rows correctly.
                    df = pd.DataFrame(result.fetchall(), columns=result.keys())
                else:
                    # If the query does not return rows (e.g., DDL, some DML, or SELECT that doesn't yield a rowset)
                    # Log a warning if DEBUG is on, as we primarily expect SELECTs
                    try:
                        from config import Config # Local import to avoid circular dependency at module level if any
                        if Config().DEBUG:
                            print(f"[DB_WARNING] Executed query did not return rows: {query[:200]}...")
                    except ImportError:
                        print(f"[DB_WARNING] Executed query did not return rows (Config not available for full debug): {query[:200]}...")
                    except Exception as e_conf:
                        print(f"[DB_WARNING] Executed query did not return rows (Error accessing Config for debug: {e_conf}): {query[:200]}...")

                    df = pd.DataFrame() # Return an empty DataFrame
                
                # It's good practice to explicitly close the result proxy if it's not fully consumed
                # by fetchall(), though with context manager for connection, it should be handled.
                # result.close()
                return df
        except Exception as e:
            # Add the problematic query to the exception message for better debugging
            raise Exception(f"Query failed: {str(e)}. Problematic Query: {query[:500]}")
